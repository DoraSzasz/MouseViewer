...
public class MouseGLSurfaceView extends GLSurfaceView implements
		MultiTouchObjectCanvas<MouseGLSurfaceView> {

	private static String TAG = "MouseViewer";
	...
	private final MultiTouchController<MouseGLSurfaceView> multiTouchController = new MultiTouchController<MouseGLSurfaceView>(
			this);

	private PointInfo mLastTouchInfo = null;
	private final PointInfo mCurrentTouchInfo = new PointInfo();
	...
	public MouseGLSurfaceView(Context context) {
		super(context);
		init();
	}

	public MouseGLSurfaceView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	private void tryPreserveEGLContext() {
		...
		try {
			Method preserveEGLContextMethod = MouseGLSurfaceView.class
					.getMethod("setPreserveEGLContextOnPause",
							new Class[] { boolean.class });
			try {
				preserveEGLContextMethod.invoke(MouseGLSurfaceView.this, true);
			}
			catch (InvocationTargetException ite) {
				Log.i(TAG,
						String.format("exception invoking setPreserveEGLContextOnPause()"));
			}
			catch (IllegalAccessException ie) {
				Log.i(TAG,
						String.format("exception accessing setPreserveEGLContextOnPause()"));
			}
		}
		catch (NoSuchMethodException nsme) {
			Log.i(TAG, String
					.format("api does not have setPreserveEGLContextOnPause()"));
		}
	}
	...
	public void postLoadDefaultDataset(final MouseViewerActivity loader,
		...
		this.queueEvent(new Runnable() {
			...
			public void run() {
				...
				if (!KiwiNative.getDatasetIsLoaded()) {
					// final int defaultDatasetIndex = KiwiNative
					// .getDefaultBuiltinDatasetIndex();
					final int defaultDatasetIndex = 1;
					MouseGLSurfaceView.this.post(new Runnable() {
						@Override
						public void run() {
	...
	@Override
	public void selectObject(MouseGLSurfaceView obj, PointInfo touchPoint) {
	...
	@Override
	public MouseGLSurfaceView getDraggableObjectAtPoint(PointInfo touchPoint) {
	...
	@Override
	public void getPositionAndScale(MouseGLSurfaceView obj, PositionAndScale pos) {
		...
		pos.set(xOff, yOff, updateScale, scale, updateScaleXY, scaleX, scaleY,
				updateAngle, angle);
	...
	@Override
	public boolean setPositionAndScale(MouseGLSurfaceView obj,
			PositionAndScale pos, PointInfo info) {
		...
		if (isMulti && mLastTouchInfo.getMultiTouchDiameter() != 0.0f) {
			scale = mCurrentTouchInfo.getMultiTouchDiameter()
					/ mLastTouchInfo.getMultiTouchDiameter();
		}

		if (isMulti) {
			angle = mCurrentTouchInfo.getMultiTouchAngle()
					- mLastTouchInfo.getMultiTouchAngle();
		}
	...
	public void loadDataset(final String filename,
			final MouseViewerActivity loader) {
		int builtinDatasetIndex = -1;
		loadDataset(filename, builtinDatasetIndex, loader);
	}

	public void loadDataset(final String filename,
			final int builtinDatasetIndex, final MouseViewerActivity loader) {

		queueEvent(new Runnable() {
			@Override
			public void run() {

				final boolean result = KiwiNative.loadDataset(filename,
						builtinDatasetIndex);
				final String errorTitle = KiwiNative.getLoadDatasetErrorTitle();
				final String errorMessage = KiwiNative
						.getLoadDatasetErrorMessage();
				...
				MouseGLSurfaceView.this.post(new Runnable() {
					@Override
					public void run() {
						loader.postLoadDataset(filename, result, errorTitle,
								errorMessage);
					}
				});
			}
